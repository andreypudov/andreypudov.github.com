...
const entry = (page: Page, t: TFunction) => (
  <div className="col-md-4" key={page.getAlbum().getTitle().getKey()}>
    <Link
      to={page.getRoute()}
      className="card bg-dark text-white gallery-image"
    >
      <img
        src={page.getAlbum().getCover().src}
        className="card-img album-cover"
        alt=""
      />
      <div className="card-img-overlay">
        <h6 className="card-title">
          {t(
            page.getAlbum().getTitle().getKey(),
            page.getAlbum().getTitle().getFallback(),
          )}
        </h6>
      </div>
    </Link>
  ...
  return (
    <Layout title={"Albums"}>
      <div className="container">
        ...
        <div className="row mb-4">{Drawings.map((d) => entry(d, t))}</div>
        ...
        <div className="row mb-4">{TimeLapses.map(videoEntry)}</div>

        <h2>{t("Contents.Articles", "Articles")}</h2>
        <div className="row">{Articles.map((a) => entry(a, t))}</div>
      </div>
    </Layout>
  );
};
