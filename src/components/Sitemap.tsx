...
const urlEntry = (page: Page, timestamp: string, locale: string) => {
  ...
  return `<url>
    ...
    ${page
      ...
      .getMedia()
      .map((media: Media) =>
        imageEntry(t(page.getAlbum().getTitle().getKey()), media),
      )
      .join("")
      .trim()}
...
const generateSitemap = (timestamp: string, locale: string) => {
  ...
  ${Articles.map((page: Page) => urlEntry(page, timestamp, locale))
    .join("")
    .trim()}
  ${Photographs.map((page: Page) => urlEntry(page, timestamp, locale))
    .join("")
    .trim()}
  ${Drawings.map((page: Page) => urlEntry(page, timestamp, locale))
    .join("")
    .trim()}
  ${TimeLapses.map((page: Page) => urlEntry(page, timestamp, locale))
    .join("")
    .trim()}
  ${Performances.map((page: Page) => urlEntry(page, timestamp, locale))
    .join("")
    .trim()}
  `;
};
  ...
  return (
    <Layout title={"Sitemap"}>
      <div className="container">
        ...
        <textarea
          style={{ width: "100%", height: "640px", border: "1px solid grey" }}
        >
          {'<?xml version="1.0" encoding="UTF-8"?>' +
            '\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">' +
            supportedLanguages
              .map((locale) => generateSitemap(timestamp, locale))
              .join("") +
            "</urlset>"}
        </textarea>
      </div>
